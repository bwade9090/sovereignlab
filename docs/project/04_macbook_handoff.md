...
- Prepared: 2026-07-16; refreshed 2026-08-25 after the bilingual KOSIS CPI owner-approval
  transition (twenty-third refresh: fifth core batch approved, owner-directed draft slice next)
...
- Session state: work unit C and its first nine ADR 0008 slices are complete and reviewable; all
  ...
  Korean/English pair, so the core is now 12/40 with no pending draft; the owner directed a
  bounded draft-only authoring slice for the frozen `kv-core-abstain-02` pair as the exact next
  outcome
...
5. Start only with section 5's owner-directed draft-only authoring slice for the frozen
   `kv-core-abstain-02` pair; do not pre-approve its new drafts or select another pair.
...
- The approved core count is exactly 12/40. The four records in
  ...
  `data/benchmark/core/core-batch-005.jsonl` are approved. No draft is pending; the remaining 28
...
and the annotations preserve the AI author `Claude AI draft`. The approved core is now 12/40, the
remaining 28 slots are unauthored and unapproved, and no draft is pending. This approval completes
the data route's four authorable pairs (`kv-core-data-01`–`kv-core-data-04`); the fifth data pair
`kv-core-data-05` stays reserved on the deliberately unauthored test-split unit. The matrix,
source bundle, rights decisions, 13 public schemas, five committed traces, and runtime source
remain unchanged.
...
1. Author only the frozen `kv-core-abstain-02` pair as a bounded draft-only slice. It is an
   abstention pair on the `train` split whose question asks for a neighboring OECD observation
   scope (Korea's normalised CLI) that has no owner-approved raw-data decision, so the gold
   behavior is abstention. Abstain pairs bind no source units; the slice uses no new evidence,
   only the committed rights catalog as the fail-closed basis.
