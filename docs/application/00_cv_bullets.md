...
- Last updated: 2026-08-27 (M2 mid-window refresh: 20/40 approved core, ADR 0008 slices 1–9 and
  work unit C complete, two combined document-plus-data drafts pending review, 79-file/1,178-test
  baseline)
...
- Building **K-VINTAGE**, a bilingual Korean/English benchmark whose gold answers depend on the
  ...
  human-reviewed core allocation is frozen and 20/40 records are reviewed and approved so far —
  ...
  is to ask for the missing as-of and abstain, and a Korean/English test-split ledger-frontier
  abstention pair asking for Korea's OECD amplitude-adjusted CLI for May 2026 using only the
  vintage available as of August 15, 2026 — a cutoff beyond the committed availability ledger's
  completeness frontier (`complete_through`, the 2026-07-17 capture instant), so the approved
  gold behavior is abstention with `cutoff_beyond_complete_through` — completing approval of
  the abstention route's five pairs. A separate Korean/English combined pair — the first
  `documents_and_data` pair, a train-split pair whose records each combine one page-anchored
  document claim over the Bank of Korea May 2026 outlook's 2026 GDP growth projection of 2.6%,
  well above the February forecast of 2.0%, with one fail-closed resolution of the demonstrably
  available July 2026 OECD Korea CLI edition 202607, value 102.66, at a shared 2026-07-09
  as-of — is complete only as two AI-authored drafts pending named human review and is not
  included in the 20/40 approved count; machine-generated revision probes will be reported
  separately.
...
- Verified the pinned Ministral 3 3B NF4/QLoRA compatibility path on a disposable A40/CUDA 13 GPU
  and maintain 1,178 tests at 100% statement/branch coverage (4,679 statements, 1,568 branches)
  across 79 formatted Python files; temporal leakage remains the planned headline metric, and no
...
- Building **SovereignLab / K-VINTAGE** (in progress): to our knowledge, for official statistics,
  ...
  state: 20/40 reviewed-and-approved bilingual core records, cutoff-filtered bilingual temporal
  ...
  partial evidence. Two additional combined document-plus-data records (`kv-core-both-01`) remain
  draft-only pending named human review and are not counted in the 20/40 approved core. The
  verified Ministral 3 3B NF4/QLoRA compatibility path is retained, and 1,178 tests pass at 100%
  statement/branch coverage; temporal leakage is the planned headline metric and no model-quality
  or briefing-performance result is claimed yet.

This version distinguishes the deterministic offline replay slice from provider/live integration,
the two pending draft records, the 18 unauthored core slots, and the not-yet-evaluated model
variants.
...
- Do not cite benchmark size until each counted test item passes schema and human review; **always report the human-reviewed core and machine-generated probes as separate counts**. The current approved core count is 20/40 — the halfway mark of the frozen matrix; of the 20 unapproved slots, two are AI-authored drafts pending named human review and 18 remain unauthored.
