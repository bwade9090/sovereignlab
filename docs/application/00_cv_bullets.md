...
- Last updated: 2026-08-25 (M2 mid-window refresh: 12/40 approved core, ADR 0008 slices 1–9 and
  work unit C complete, two OECD scope abstention drafts pending review, 75-file/1,153-test
  baseline)
...
- Building **K-VINTAGE**, a bilingual Korean/English benchmark whose gold answers depend on the
  ...
  human-reviewed core allocation is frozen and 12/40 records are reviewed and approved so far —
  ...
  current-account snapshots, and a Korean/English data pair over the 2026-07-17 KOSIS national
  CPI snapshot. A separate Korean/English abstention pair asking for Korea's OECD normalised CLI —
  a neighboring measure outside the sole owner-approved OECD raw-data scope, so the drafted gold
  behavior is a rights-based abstention — is complete only as two AI-authored drafts pending
  named human review and is not included in the 12/40 approved count; machine-generated revision
  probes will be reported separately.
...
- Verified the pinned Ministral 3 3B NF4/QLoRA compatibility path on a disposable A40/CUDA 13 GPU
  and maintain 1,153 tests at 100% statement/branch coverage (4,679 statements, 1,568 branches)
  across 75 formatted Python files; temporal leakage remains the planned headline metric, and no
...
- Building **SovereignLab / K-VINTAGE** (in progress): to our knowledge, for official statistics,
  ...
  state: 12/40 reviewed-and-approved bilingual core records, cutoff-filtered bilingual temporal
  ...
  partial evidence. Two additional OECD scope abstention records remain draft-only pending named
  human review and are not counted in the 12/40 approved core. The verified Ministral 3 3B
  NF4/QLoRA compatibility path is retained, and 1,153 tests pass at 100% statement/branch
...
the two pending draft records, the 26 unauthored core slots, and the not-yet-evaluated model
variants.
...
- Do not cite benchmark size until each counted test item passes schema and human review; **always report the human-reviewed core and machine-generated probes as separate counts**. The current approved core count is 12/40; of the 28 unapproved slots, two are AI-authored drafts pending named human review and 26 remain unauthored.
