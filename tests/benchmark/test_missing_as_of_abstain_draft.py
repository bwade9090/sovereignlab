"""Contract and evidence checks for the draft kv-core-abstain-04 pair."""
...
DRAFT_PATH = ROOT / "data" / "benchmark" / "drafts" / "core-draft-008.jsonl"
...
def test_draft_pair_matches_the_frozen_allocation_and_has_no_review_metadata() -> None:
    ...
    pair = next(item for item in matrix.pairs if item.pair_id == "kv-core-abstain-04")
...
def test_approved_core_remains_sixteen_and_drafts_are_separate() -> None:
    matrix = CoreAuthoringMatrix.model_validate_json(MATRIX_PATH.read_text(encoding="utf-8"))
    approved = tuple(
    ...
    assert matrix.target_record_count - len(approved) - len(drafts) == 22


def test_questions_omit_the_as_of_cutoff_and_reasons_demand_it() -> None:
    korean, english = _draft_records()

    assert "일 현재" not in korean.question
    assert "기준 시점" not in korean.question
    assert "as of" not in english.question.lower()
    assert "at the time" in english.question
    assert "당시" in korean.question
    assert "effective_as_of" in korean.abstention_reason
    assert "effective_as_of" in english.abstention_reason
...
def test_bilingual_claims_are_parallel_and_leak_no_observation_value() -> None:
    ...
    assert "진폭조정 CLI" in korean.question
    assert "2026년 5월" in korean.question
    assert "기준 시점" in korean.abstention_reason
    assert "시간 누출" in korean.abstention_reason
    assert "기권" in korean.abstention_reason
    assert "amplitude-adjusted CLI" in english.question
    assert "May 2026" in english.question
    assert "cutoff" in english.abstention_reason
    assert "temporal leakage" in english.abstention_reason
...
def test_the_same_request_resolves_once_an_explicit_cutoff_is_supplied() -> None:
